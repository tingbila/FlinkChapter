...
public class State_Keyed_Bug {
    public static void main(String[] args) throws Exception {
        ...
        DataStream<String> processDataStream = keyedStream.process(new KeyedProcessFunction<String, WaterSensor, String>() {
            private Integer lastVc = 0;  //此时lastVc多个key所共享

            @Override
            public void processElement(WaterSensor value, Context ctx, Collector<String> out) throws Exception {
                out.collect("当前数据所属的key=" + ctx.getCurrentKey() + ",保存的上一次水位值是=" + lastVc);
                lastVc = value.getVc();
            }
        });

        processDataStream.print();
        env.execute("State_Keyed_Bug");
    }
}
